...
function showMask(options) {
    ...
    $("<div class=\"wait-mask-msg\"></div>").html(msg).appendTo(wrap).css({
        display: "block",
        left: (width - $("div.wait-mask-msg", wrap).outerWidth()) / 2,
        top: (height - $("div.wait-mask-msg", wrap).outerHeight()) / 2 - 5
    });
}
...
function blurInput(obj, options) {
    ...
    ajaxEx({
        url: options.url, data: data, success: function (data) {
